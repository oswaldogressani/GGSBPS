...
write.csv(file = "ScenarioC.csv", subperf, row.names = FALSE)
...
tarfun <- ggplot(data = datatar, aes(x = x, y = y)) +
  ...
  theme_classic() + xlim(0,1) + ylim(0,12)
...
png(file = "ScenarioC.png", width = 1000, height = 500)
tarfun
dev.off()

pdf(file = "ScenarioC.pdf", width = 12.3, height = 6)
tarfun
dev.off()
